...
class informe_venta_facturas:

    def __init__(self, datos):
        self.datos_conexion = datos

        arch = archivo("informe_venta_facturas")
        self.obj = arch.get_object

        self.obj("ventana").set_position(1)
        self.obj("ventana").set_title("Informes de Facturas de Venta")
        ...
        self.obj("label15").set_visible(False)
    ...
    def on_btn_aceptar_clicked(self, objeto):
        ...
        informe_vista(self, "Facturas de Venta")
    ...
    def on_cmb_changed(self, objeto):
        ...
        if objeto == self.obj("cmb_cliente_doc"):
            ...
            self.obj("label15").set_visible(estado)
...
##### Ventana de Vista Previa ##########################################
    ...
    def config_grilla(self, grilla):
        ...
        col4 = columnas("Nro. Doc. Cliente", celda0, 4, True, 100, 150)
        col4.set_sort_column_id(4)
        col5 = columnas("Cliente", celda1, 5, True, 100, 250)
    ...
    def cargar_grilla(self, grilla, barraestado):
        ...
        if self.obj("chk_02").get_active():  # Nro. de Factura
            if len(condicion) > 0:
                condicion += " AND "
            condicion += "NroFactura >= " + str(self.obj("txt_nro_ini").get_value_as_int()) + \
                " AND NroFactura <= " + str(self.obj("txt_nro_fin").get_value_as_int())
        ...
        if self.obj("chk_05").get_active():  # Tipo de Factura
            if len(condicion) > 0:
                condicion += " AND "
            condicion += "idTipoFactura = " + str(self.idTipoFact)

        if self.obj("chk_06").get_active():  # Fecha
            if len(condicion) > 0:
                condicion += " AND "
            condicion += "FechaHora BETWEEN '" + self.fecha_ini + "%'" + \
                " AND '" + self.fecha_fin + "%'"

        if self.obj("chk_07").get_active():  # Total
            if len(condicion) > 0:
                condicion += " AND "
            condicion += "Total "
        ...
        # Obtener datos de Factura de Venta
        conexion = conectar(self.datos_conexion)
        cursor = consultar(conexion, "NroTimbrado, NroEstablecimiento, " +
            "NroPuntoExpedicion, NroFactura, FechaHora, TipoFactura, NroDocCliente, RazonSocial, " +
            "Total", "facturaventas_s", " WHERE " + condicion)
    ...
    def preparar_pdf(self, grilla):
        ...
        ubicacion_archivo = dialogo_guardar("Facturas de Venta")
        ...
        if ubicacion_archivo is not None:
            ...
            story.append(Par("Facturas de Venta", cabecera))
            story.append(Spacer(0, 20))

            if self.obj("chk_01").get_active():  # Nro. de Timbrado
                story.append(Par("Facturas por el Timbrado Nro. <b>" +
            ...
            lista = [[Par("Nro. Timbrado", head), Par("Nro. Factura", head),
                Par("Fecha de Expedición", head), Par("Tipo de Factura", head),
                Par("Nro. Doc. Cliente", head), Par("Cliente", head), Par("Total", head)]]
