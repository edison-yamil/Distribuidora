...
class informe_venta_nota_creditos:

    def __init__(self, datos):
        self.datos_conexion = datos

        arch = archivo("informe_venta_nota_creditos")
        self.obj = arch.get_object

        self.obj("ventana").set_position(1)
        self.obj("ventana").set_title("Informes de Notas de Crédito por Ventas")
    ...
    def on_btn_aceptar_clicked(self, objeto):
        ...
        informe_vista(self, "Notas de Crédito por Ventas")
...
##### Ventana de Vista Previa ##########################################
    ...
    def config_grilla(self, grilla):
        ...
        col1 = columnas("Nro. Nota de Crédito", celda0, 1, True, 100, 200)
    ...
    def cargar_grilla(self, grilla, barraestado):
        ...
        if self.obj("chk_02").get_active():  # Nro. de Nota de Credito
            if len(condicion) > 0:
                condicion += " AND "
            condicion += "NroNotaCredito >= " + str(self.obj("txt_nro_ini").get_value_as_int()) + \
                " AND NroNotaCredito <= " + str(self.obj("txt_nro_fin").get_value_as_int())
        ...
        # Obtener datos de Nota de Credito por Ventas
        conexion = conectar(self.datos_conexion)
        cursor = consultar(conexion, "NroTimbrado, NroEstablecimiento, " +
            "NroPuntoExpedicion, NroNotaCredito, FechaHora, NroDocCliente, RazonSocial, " +
            "Total", "notacreditoventas_s", " WHERE " + condicion)
    ...
    def preparar_pdf(self, grilla):
        ...
        ubicacion_archivo = dialogo_guardar("Notas de Crédito por Ventas")
        ...
        if ubicacion_archivo is not None:
            ...
            story.append(Par("Notas de Crédito por Ventas", cabecera))
            story.append(Spacer(0, 20))

            if self.obj("chk_01").get_active():  # Nro. de Timbrado
                story.append(Par("Notas de Crédito por el Timbrado Nro. <b>" +
            ...
            if self.obj("chk_02").get_active():  # Nro. de Nota de Credito
                story.append(Par("Números de Nota de Crédito entre <b>" +
                str(self.obj("txt_nro_ini").get_value_as_int()) + "</b> y <b>" +
                str(self.obj("txt_nro_fin").get_value_as_int()) + "</b>", parrafo))

            if self.obj("chk_03").get_active():  # Cliente
                story.append(Par("Notas de Crédito del Cliente <b>" +
                self.obj("txt_cliente_02").get_text() + "</b>", parrafo))

            if self.obj("chk_04").get_active():  # Vendedor
                story.append(Par("Notas de Crédito del Vendedor <b>" +
                self.obj("txt_vendedor_02").get_text() + "</b>", parrafo))

            if self.obj("chk_05").get_active():  # Fecha
                story.append(Par("Notas de Crédito expedidas entre el " +
            ...
            if self.obj("chk_06").get_active():  # Total
                ...
                story.append(Par("Notas de Crédito expedidas por un Monto " + tipo + " <b>" +
                str(self.obj("txt_total_ini").get_value()) + " </b>" + segundo, parrafo))

            story.append(Spacer(0, 20))

            lista = [[Par("Nro. Timbrado", head), Par("Nro. Nota de Crédito", head),
            ...
            doc = SimpleDocTemplate(
                ...
                allowSplitting=1,
                title="Notas de Crédito por Ventas",
