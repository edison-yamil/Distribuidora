...
class funcion_abm:
    ...
    def on_btn_guardar_clicked(self, objeto):
        ...
        sql = cod + ", " + cont + ", " + str(self.idMotivoSancion) + ", " + \
    ...
    def verificacion(self, objeto):
        ...
        or self.idTipoDoc == -1 or self.idMotivoSancion == -1:
            estado = False
        else:
    ...
    def on_cmb_changed(self, objeto):
        ...
        if active > -1:
            if objeto == self.obj("cmb_tipo_doc"):
                ...
                self.idMotivoSancion = model[active][0]
        else:
            if objeto == self.obj("cmb_tipo_doc"):
                tipo = "Tipos de Documentos"
            elif objeto == self.obj("cmb_motivo"):
                tipo = "Motivos de Sanción"
    ...
    def focus_in_event(self, objeto, evento):
        if objeto in (self.obj("txt_01"), self.obj("txt_01_2")):
            ...
            tipo = "la Fecha de Inicio del Periodo de Sanción"
        elif objeto == self.obj("txt_06"):
            tipo = "la Fecha de Finalización del Periodo de Sanción"
            ...
            if objeto == self.obj("txt_00"):
                ...
                and len(self.obj("txt_06").get_text()) > 0:
                    ...
                    "'" + self.fechaini + "'", ">", "'" + self.fechafin + "'"):
                        self.estadoguardar(False)
                        objeto.grab_focus()
                        self.obj("barraestado").push(0, "La Fecha de Inicio del Periodo de Sanción NO puede ser posterior a la de Finalización.")
                    else:
...
def config_grilla(self):
    ...
    col10 = Op.columnas("Motivo de Sanción", celda1, 10, True, 200)
...
def cargar_grilla(self):
    ...
    cursor = Op.consultar(conexion, "idSancion, NroContrato, idEmpleado, " +
        "idTipoDocumento, NroDocumento, NombreApellido, FechaNacimiento, " +
        "Edad, Cargo, idMotivoSancion, MotivoSancion, FechaInicio, FechaFin, " +
...
def seleccion(self):
    try:
        ...
        ini = seleccion.get_value(iterador, 11)
        fin = seleccion.get_value(iterador, 12)

        self.origen.txt_cod_snc.set_text(cod)
        self.origen.txt_per_snc.set_text("Desde " + ini + " hasta " + fin)
